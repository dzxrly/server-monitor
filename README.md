...
- Configurable top CPU, memory, and GPU process counts shown below their resource panels.
...
- Node.js 24 or newer.
- npm 11 or newer.
- A deployed [Server Monitor backend](https://github.com/dzxrly/server-monitor/tree/backend-dev) on each monitored host.
...
- `High-usage processes per resource` controls the `processLimit` query parameter and accepts `1`–`50`.
