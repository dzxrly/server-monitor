<template>
  <q-card
    class="server-edit-dialog-wrapper full-width bg-card-color rounded-borders"
  >
...
const serverConfig = reactive<ServerConfig>(
  configStore.config.serverListConfig.find(
    (server) => server.uniqueId === props.serverUniqueId
  ) as ServerConfig
);
...
const serverUrlPrependUrlSelect = ref(
  serverConfig.serverUrl.split('://')[0] + '://'
);
