...
- 顯示每個邏輯核心的 CPU 佔用與頻率，使用純色背景和流暢的佔用過渡動畫，並遵循系統的減少動態效果設定。
- 同時顯示記憶體和 Swap 的使用率、已用/總容量，以及可用記憶體和剩餘 Swap 容量。
- 首頁儲存摘要優先顯示 Linux 的 `/` 或 Windows 的 `C:`；未回報系統磁碟區時，顯示已回報磁碟區中使用率最高的一項。
- 硬碟詳細資料依總容量由大到小排列掛載磁碟區，容量相同時依掛載路徑排序。
- 硬碟容量依數值大小自動選擇 B、KB、MB、GB、TB、PB 或 EB；記憶體、Swap 和顯示記憶體使用設定中的單位，可選範圍為 B 到 EB。
